...
- **Web slices (feature/context-library-web):** #48 MERGED (Andrii), so B/C unblocked.
  ...
  - **Slice C (review wizard, D-137 steps 1-2) — next.** Needs a `Stepper` kit primitive. Wizard at `/sources/:id/review`: step 1 confirm/adjust placement (proposedContextId / pick / create-new via slice-A modal), step 2 keep/discard items (`GET /sources/:id/items`), submit `POST /sources/:id/review` (`ReviewApprovalRequestSchema`). `useWsEvent('classification_ready')` to live-render the review affordance. Step 3 (ledger) = D-136 fast-follow, out of scope.
  - **Slice D (chat) — after C.** markdown-render dep chosen then. Note: prod build already warns on >500 kB single bundle — code-split the markdown/highlighter dep at D.
