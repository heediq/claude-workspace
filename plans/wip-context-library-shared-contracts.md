...
Both are unblocked now that Step 4 is fully merged; neither has been planned in detail yet (Step 2 of the dev workflow: plan with QA scenarios, tests, UI conformance, WS event, rollback, risk/regression — before any code):
- **4c-ii (Chat, api+infra)** — smaller, backend-only, continues the API-side momentum; needed before Step 5's chat UI can do anything real.
- **Step 5 (Web)** — larger, frontend-only; Context tree/library + review wizard don't need chat and can ship without it, but the chat *panel* in Step 5 would be UI-only (no working backend) until 4c-ii lands.
Ask Andrii which to start; don't assume.
