...
## Step 4 — API (`heediq-api`) — 🟡 plan approved, infra + shared addendum PRs open, API routes next
...
- **4b-infra · `heediq-infra` ApiStack grants — ✅ DONE → PR open** [heediq-infra#61](https://github.com/heediq/heediq-infra/pull/61) (branch `feature/context-library-api-grants`). Wires the `heediq-api` Lambda's IAM read/write grants + env vars for all 6 Context Library tables (previously only `SummarizationStack` had them). 190 tests. Doesn't block API code work, only the eventual deploy — merge anytime.
...
- **4b-api · API routes — ✅ DONE → PR open** [heediq-api#42](https://github.com/heediq/heediq-api/pull/42) (branch `feature/context-library-api`, bumped to 0.15.1). `src/routes/contexts.ts` (new — GET list/tree, POST, GET :id, PATCH :id, DELETE :id, writer computes `scopeKey`/`domainCreatedAt`) + `POST /sources/:id/review` review-approval endpoint on `sources.ts` (sets kept items' `contextId`+`status='kept'`, others `status='discarded'`, Source `classification='approved'`). Every mutating route: `requirePermission` + `auditWriter` (`context`/`extractedItemReview` payloads). 235 unit tests, integration suites green (contexts 12/12, sources 9/9). `heediq-api/README.md` updated (Key Files, Data Flow, Contracts, Dependencies, Testing, Gotchas). Also fixed a pre-existing latent bug: `sources.ts`'s `resolveOwnerEmail` `'unknown'` fallback failed `@heediq/shared`'s email-format audit validation, 500ing PATCH/DELETE for any unseeded user — root-caused and fixed (D-113) by seeding a real user in the integration tests that exercise those paths.
...
- **4c · Cross-org grants + chat — ⬜ (fast-follow, D-142)**: grant issuance/revoke routes + per-request cross-org authorization middleware (authorize against a live unexpired grant every request — heavily isolation-tested); `heediq-chat` SQS queue + Lambda worker (infra, D-139); conversations/messages endpoints + chat-job enqueue (api). Plan separately once 4b lands.

**Still open, non-blocking:** [heediq-infra#61](https://github.com/heediq/heediq-infra/pull/61) (ApiStack IAM grants for the 6 Context Library tables) awaits Andrii's merge — needed before deploy, not before further API/web code work.
**PR #42 also fixed two pre-existing latent bugs, root-caused per D-113 (unrelated to Context Library, picked up while getting the integration gate green):** `sources.ts`'s `resolveOwnerEmail` `'unknown'` fallback (invalid email format, 500ed PATCH/DELETE for any unseeded acting user) and `tests/integration/audit-log.test.ts`'s `seedEntry` (sk diverging from timestamp — deterministic, not a race, exposed by `requirePermission`'s D-114 denial-audit side effect). Both fixed, regression-tested, `pnpm run test:pre-pr` + `test:integration` fully green (235 unit / 53 integration tests). Also added `scripts/wipe-user.sh` dev utility (D-099 clean-slate identity retesting).
