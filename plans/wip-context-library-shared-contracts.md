...
- **Web slices (feature/context-library-web):** #48 MERGED (Andrii), so B/C unblocked.
  ...
  - **Slice D (chat) — next / LAST.** Context chat panel streaming via `useWsEvent('chat_delta'/'chat_complete'/'chat_failed')`, held to 04-loading-and-feedback §6 (token stream, pre-first-token thinking indicator, scroll-aware auto-scroll + jump-to-latest, Stop/Retry, incremental markdown w/ highlight, per-turn copy, auto-grow composer, one-turn guard). Backend: `POST/GET /conversations?contextId=`, `GET/POST /conversations/:id/messages`. **Claude picks the markdown/highlight stack (mobile-first best-practice) + code-splits it** (prod build already warns >500 kB single bundle). Per **D-147** generalize `scratchpad/chat-e2e.mjs` → committed `tests/e2e/`. Then the whole branch gets ONE PR (user's call — all slices on `feature/context-library-web`).
