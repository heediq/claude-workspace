...
- **4b · API impl — ⬜** (branch `feature/context-library-api`, bumped to 0.15.0): scope narrowed to **Contexts CRUD (+ tree; writer computes `scopeKey`/`domainCreatedAt`) + review-approval endpoints only** (set `contextId`+`kept`, `classification='approved'`) — every mutating route: `requirePermission` + `auditWriter` (extend `AuditPayloadMap`) + frontend `<Can>` (D-107). Small infra companion PR needed alongside: wire `heediq-api`'s Lambda IAM grants + env vars for the 6 Context Library tables in `ApiStack` (currently only `SummarizationStack` has them, from Step 3) — tables/SSM params already exist on `FoundationStack` (Step 2), just not consumed by the API Lambda yet.
