...
## Step 3 — Ingest (combined classify+extract, D-130/D-133) — ✅ DONE → 3 PRs open
PRs (merge **infra #60 first** — grants/stream/pusher shell): infra [heediq-infra#60](https://github.com/heediq/heediq-infra/pull/60) · worker [heediq-worker-summarization#16](https://github.com/heediq/heediq-worker-summarization/pull/16) · api [heediq-api#41](https://github.com/heediq/heediq-api/pull/41).
...
## Step 4 — API (`heediq-api`) — 🟡 addendum done, API impl next
- **4a · `@heediq/shared` 0.15.0 addendum — ✅ DONE → PR [heediq-shared#40](https://github.com/heediq/heediq-shared/pull/40)** (branch `feature/context-library-shared-visibility-grants`). `ContextVisibility` + `ContextGrantAccess` enums; `ContextSchema` +`visibility`(default personal)/+`groupId?` (required iff group); `ContextGrantSchema` (regulated cross-org share, `expiresAt` required, `ownerOrgId` homing); permission catalog appends `context:read/create/update/delete/share` (members get CRUD, `context:share` admin-gated). Additive, 217 tests. **Publishes on the release PR to `main`** (like 0.14.0) — the API build consumes 0.15.0 once published.
- **4b · API impl — ⬜** (branch `feature/context-library-api`, already open w/ the Step-2 create-tables mirror): bump 0.15.0. Contexts CRUD (+ tree; writer computes `scopeKey`/`domainCreatedAt`), review-approval endpoints (set `contextId`+`kept`, `classification='approved'`), conversations/messages endpoints, chat-job enqueue, grant issuance/revoke routes + per-request cross-org **authorization middleware** (authorize against a live unexpired grant every request — heavily isolation-tested). Every mutating route: `requirePermission` + `auditWriter` (extend `AuditPayloadMap`) + frontend `<Can>` (D-107). Add group-scoped Contexts to the ingest classifier follow-up here (RBAC group lookup).
