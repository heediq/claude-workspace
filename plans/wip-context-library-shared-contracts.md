...
- **Web slices (feature/context-library-web):** #48 MERGED (Andrii), so B/C unblocked.
  ...
  - **Slice B (source detail + items) / C (review wizard) — next**, both consume `GET /sources/:id/items` (#48). D is chat; markdown-render dep chosen at D. Note: prod build already warns on >500 kB single bundle — code-split the markdown/highlighter dep at D.
