...
| 5 | Audit log viewer | `GET /org/audit-log` (cursor-paginated, filterable); `/org/audit-log` route in `heediq-web` | **In progress** — `heediq-infra` grant done, PR open ([#51](https://github.com/heediq/heediq-infra/pull/51), 2026-07-10); `heediq-api` route + `heediq-web` page not started | `feature/audit-log-viewer-infra`/`-api`/`-web`, see `wip-audit-log-viewer.md` |
