...
| 4 | Frontend RBAC UI | `heediq-web` role/group management screens; `usePermissions`/`<Can>` wrappers wired into existing screens; new `heediq-api` `GET /me` `effectivePermissions` + `GET /api/v1/users`; `sources.ts` off legacy `role` check | Code + tests + both READMEs committed, pushed, PRs open: [heediq-api#26](https://github.com/heediq/heediq-api/pull/26), [heediq-web#26](https://github.com/heediq/heediq-web/pull/26). Awaiting review/merge. | `feature/rbac-frontend-ui-api` (heediq-api), `feature/rbac-frontend-ui` (heediq-web) |
