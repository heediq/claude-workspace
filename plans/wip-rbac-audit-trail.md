...
| 4 | Frontend RBAC UI | `heediq-web` role/group management screens; `usePermissions`/`<Can>` wrappers wired into existing screens; new `heediq-api` `GET /me` `effectivePermissions` + `GET /api/v1/users`; `sources.ts` off legacy `role` check | **Done** — [heediq-api#26](https://github.com/heediq/heediq-api/pull/26) merged `65b939f`, [heediq-web#26](https://github.com/heediq/heediq-web/pull/26) merged `803fb44` (both 2026-07-09) | — |
| 5 | Audit log viewer | `GET /org/audit-log` (cursor-paginated, filterable); `/org/audit-log` route in `heediq-web` | **In progress** — all 3 steps built, PRs open: [heediq-infra#51](https://github.com/heediq/heediq-infra/pull/51), [heediq-api#27](https://github.com/heediq/heediq-api/pull/27), [heediq-web#27](https://github.com/heediq/heediq-web/pull/27) (none merged, 2026-07-10) | `feature/audit-log-viewer-infra`/`-api`/`-web`, see `wip-audit-log-viewer.md` |
