...
| 2 | Role/group CRUD + audit write path | New `heediq-api` routes for roles/groups/assignments; `writeAuditEvent` helper; per-resource-type payload resolution | In progress — plan not yet approved | `feature/rbac-role-group-crud` (heediq-shared, heediq-api, heediq-infra) / `wip-feature-rbac-role-group-crud.md` |
