...
**Mechanism correction (2026-08-07, D-157):** the clause "carried via the SQS message's `tier` attribute → EventBridge Pipe filter" no longer holds. Routing is now done in a dispatcher Lambda that reads `tier` from the message body and picks the task def in code; there is no `tier` SQS attribute and no Pipe. The access-control-at-API *principle* below is unchanged.
**Supersedes:** D-005 (mechanism: CPU routing → API access control; model assignments free=small / paid=large-v3+pyannote unchanged)
**Superseded by:** D-157 (routing mechanism only — access-control-at-API principle unchanged)
...
**Area:** Architecture / Infra
**Decision:** On SIGTERM (Spot reclamation), the transcription worker writes `status=retrying` to `heediq-jobs` and explicitly re-sends the original `TranscriptionJobMessage` to the `heediq-transcription` SQS queue before exiting. The transcription task role is granted `sqs:SendMessage` on `heediq-transcription` (previously only granted on `heediq-summarization`).
**Why:** D-059's original retry text ("lets SQS visibility timeout expire and re-enqueue") assumed the worker itself was the SQS consumer. The actual architecture has a separate SQS consumer that deletes the message as soon as it hands the job to ECS `RunTask`, before the worker container even starts — so by the time a worker could catch SIGTERM, there is no visibility timeout left to expire. Explicit re-enqueue is the correct equivalent under the one-RunTask-per-job architecture.
**Mechanism correction (2026-08-07, D-157):** the original text said the re-enqueue preserved a `tier` message attribute "so the EventBridge Pipe's filter re-routes it," and named Pipes as the consumer. Both are retired by D-157 — the consumer is now a dispatcher Lambda and there is no `tier` attribute; the worker re-sends to the single `heediq-transcription` queue and the Lambda re-routes by the `tier` in the message body. The explicit-re-enqueue-on-SIGTERM *principle* here is unchanged.
...
**Superseded by:** D-157 (re-enqueue/consumer mechanism only — explicit-re-enqueue-on-SIGTERM principle unchanged)
...
### D-157 · SQS → dispatcher Lambda → ECS RunTask; retire EventBridge Pipes for transcription (2026-08-07) — Locked
**Area:** Architecture / Infra
**Decision:** The transcription SQS→GPU consumer is a small **dispatcher Lambda** (SQS event source on the single `heediq-transcription` queue), not EventBridge Pipes. The Lambda reads the message, selects the free/paid task definition by the message's `tier`, and calls `ecs:RunTask` on the EC2 GPU capacity provider with the `SQS_MESSAGE_BODY` container override. This retires **both** tier Pipes and the `tier` SQS **message attribute** (routing is now in code, not a Pipe filter): **one queue, no filters, no per-tier queue split.** The API keeps enqueuing to the single queue with `tier` in the message body (D-060 gating unchanged); the D-066 Spot re-enqueue targets that same single queue. The stale S3 `OBJECT_CREATED` → transcription-queue notification (a pre-D-023 Fargate relic that enqueued attribute-less messages) is removed. The dispatcher lives in `heediq-infra` (pure infra glue, no business logic); its role holds `ecs:RunTask` + `iam:PassRole` (task+execution roles) — the same privilege the Pipe role held, on a dedicated internal role, not the public API Lambda.
**Why:** Two Pipes polling one queue with mutually-exclusive filters silently dropped jobs: EventBridge Pipes *deletes* filter-rejected SQS messages, so whichever Pipe won `ReceiveMessage` first consumed the message — a free job vanished whenever the paid Pipe grabbed it first (~50%), producing the D-156 audio-smoke symptom (message consumed & deleted, zero RunTask, zero ASG scaling, no error). The root cause is a Pipe's inability to branch on message content: its only routing lever is a filter, and the filter's delete-on-miss is the failure. A dispatcher Lambda branches in code, so one queue serves both tiers with no filter and no silent-drop class. It also gains what Pipes lacked on a path users actively wait on: structured logs (D-093) — this bug becomes one ERROR line instead of a 600s silent timeout — and real DLQ semantics via the queue's existing `maxReceiveCount:3 → heediq-transcription-dlq` (Pipes ignored SQS redrive). Cost is ~one small Lambda's code + tests + IAM; access-control-at-API (D-060) and explicit-re-enqueue-on-SIGTERM (D-066) principles are unchanged.
**Supersedes:** D-023 (SQS→ECS consumer: EventBridge Pipes → dispatcher Lambda; presign→S3→SQS upload flow unchanged — D-023 already archived, chain updated there), D-060 (routing mechanism only — access-control-at-API principle, model assignments, single cluster/ASG unchanged), D-066 (re-enqueue mechanism only — still the single `heediq-transcription` queue; explicit-re-enqueue-on-SIGTERM principle unchanged)         **Superseded by:** —
**Related code:** `heediq-infra/lib/transcription/transcription-stack.ts` (dispatcher Lambda + SQS event source), `heediq-infra/lib/foundation/storage.ts`, `heediq-infra/README.md`, `heediq-api/src/routes/sources.ts`, `heediq-worker-transcription/src/sqs_client.py`

### D-158 · Active docs describe only what currently exists; history/obsolete content lives in archives (2026-08-07) — Locked
**Area:** Process
**Decision:** Every **active** doc — code READMEs, `memory/codebase/*`, `memory/business/architecture.md` / `product.md`, `DECISIONS.md` area files — describes only the solution that exists right now, with no "we used to…" history, obsolete mechanism, or reference to a file/resource/flow/component that no longer exists. When a solution is replaced, descriptive docs are **rewritten to the new reality and the old text deleted** (they carry no supersession trail). Decisions are the sole exception that keeps a "why it changed" trail: a fully-superseded decision moves verbatim to `DECISIONS_ARCHIVE.md`; a partially-superseded one stays active with a dated mechanism-correction note + `Superseded by:` pointer. Enforced by the session coherence check (items 1 & 4) and by a periodic cross-repo sync (`rules/10-consistency-check.md`) that sweeps all repos' docs against the code and archives/deletes anything obsolete.
**Why:** A doc that describes a retired mechanism or names something gone from the code is worse than no doc — it actively misleads and erodes trust in the memory system. Separating "how it works now" (descriptive docs, no history) from "why it changed" (the decisions trail) keeps active reading paths lean and truthful while preserving rationale where it belongs.
**Supersedes:** — (codifies and sharpens the existing archiving discipline in `rules/09-decisions.md` and the coherence check in `rules/08-memory.md`)         **Superseded by:** —
**Related code:** `rules/08-memory.md` (§ Active docs describe only what currently exists), `rules/06-documentation.md`, `rules/09-decisions.md`, `rules/10-consistency-check.md`
