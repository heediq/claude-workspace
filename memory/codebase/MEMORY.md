...
- **Product analytics broadening (D-154, client + server LIVE in dev, 2026-08-06)** — shared id-prop contract in `heediq-shared/src/analytics.ts` (`@heediq/shared@0.15.6`); client taxonomy (~38 events, org group via `setGroup`) in heediq-web. Server choke-point in heediq-api: `heediq-api/src/lib/analytics.ts` (fail-safe/latency-bounded Amplitude Node emit helper wrapping the shared `buildServerAnalyticsEvent`) emits `source_processing_completed` from `ws-pusher.ts` (terminal job status), `user_provisioned` from `auth-provision.ts` (first login), `login_completed` from `auth-trigger-post-authentication.ts`. Infra: `heediq-infra/lib/shared/analytics-env.ts` gates `AMPLITUDE_API_KEY` on `-c analytics=true`, resolving SSM `/heediq/analytics/amplitude-api-key` (shared neutral param — same key heediq-web bakes into `VITE_AMPLITUDE_API_KEY`, so client + server land in one Amplitude project) at deploy onto the 3 emitting Lambdas only. **All three merged to `develop` + deployed to dev with `-c analytics=true`; verified the key is set on the 3 emitters and absent on non-emitters.** Staging/prod stay opt-out (no flag, param not provisioned there). Owed: Amplitude-UI validation of the cross-service join. (E2E was re-scoped by D-156 — single full real-backend suite in heediq-api, gated at promote-to-staging; the D-155 mocked browser tier was dropped.) Tracked in `plans/wip-analytics-e2e-d154-d155.md`. Follow-ups: worker-side emission + session-id correlation. See D-154.
- **E2E (D-156, supersedes D-155)** — a **single full real-backend suite** (real Cognito/API/WS + data teardown) in `heediq-api/tests/e2e/`, gated at **promote-to-staging** (push to `main` → job `e2e` in `heediq-api/.github/workflows/deploy.yml`, runs against the live **dev** stack, `deploy-staging` `needs: [build, e2e]`) and **locally on demand** — never per-PR, never on the dev deploy. D-155's mocked-backend browser tier was built on the `heediq-web` `feature/e2e-tier1-flows` branch but **dropped (PR #53 closed, never merged)**; that `heediq-web/e2e/` mocked harness + the CI-on-PR `e2e` job are not shipped. Suite = `full-loop-smoke.mjs` (deterministic text path, the gate default via `pnpm e2e`), `audio-smoke.mjs` (opt-in GPU transcription path, synth tone WAV, `pnpm e2e:audio`), `chat-smoke.mjs` (heediq-chat). Shared token helper `tests/e2e/lib/auth.mjs` (`resolveIdToken()` — ID_TOKEN/TOKEN_FILE override else Cognito `USER_PASSWORD_AUTH` provision, no AWS creds; verbatim-duplicated into heediq-chat, cross-repo). Absorbs the D-147 dev smokes. k6 stress-test wiring still pending. See `heediq-api/tests/e2e/README.md`, D-156.
