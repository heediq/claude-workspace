...
- **Product analytics broadening (D-154, client + server done)** — shared id-prop contract in `heediq-shared/src/analytics.ts` (`@heediq/shared@0.15.6`); client taxonomy (~38 events, org group via `setGroup`) on `feature/analytics-taxonomy` (heediq-web, not yet merged). Server choke-point on `feature/analytics-server-emit` (heediq-api, not yet merged): `heediq-api/src/lib/analytics.ts` (fail-safe/latency-bounded Amplitude Node emit helper wrapping the shared `buildServerAnalyticsEvent`) emits `source_processing_completed` from `ws-pusher.ts` (terminal job status), `user_provisioned` from `auth-provision.ts` (first login), `login_completed` from `auth-trigger-post-authentication.ts`. Infra wiring on a `heediq-infra` feature branch (not yet merged): `lib/shared/analytics-env.ts` gates `AMPLITUDE_API_KEY` on `-c analytics=true`, resolving SSM `/heediq/api/amplitude-api-key` at deploy onto the 3 emitting Lambdas only. All three repos' `test:pre-pr` green. Tracked in `plans/wip-analytics-e2e-d154-d155.md`. Follow-ups: worker-side emission + session-id correlation. See D-154.
