...
- **Upstream**: `@heediq/shared` analytics contract (`buildServerAnalyticsEvent` + id-prop types + cross-service event names); Amplitude (external); `custom:accountId`/`custom:orgId` ID-token claims (identity/join keys); `heediq-sources`/`heediq-users` reads for the server emit sites; SSM `/heediq/analytics/amplitude-api-key` (shared neutral param — same key heediq-web bakes into `VITE_AMPLITUDE_API_KEY`)
