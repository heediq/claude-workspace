...
### Context Library (D-124–D-140) — build in progress (§11 of `plans/context-library-spec.md`)
- **Upstream**: `@heediq/shared` Context Library contracts (**landed** on branch `feature/context-library-shared-contracts`, v0.14.0: `Domain`/`DOMAIN_PROFILES`, `Context`/`ExtractedItem`/`DecisionLedgerEntry`/`ProposedClassification`, Source review fields, `classification_ready`/`chat_delta`/`chat_complete` WS events); later steps depend on new infra tables (`heediq-contexts` + `by-org` GSI, ExtractedItem/ledger/conversations stores) not yet built.
